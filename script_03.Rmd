...
group_by(title, id, name, department) %>%
  summarize(u_grad = sum(u_grad)) %>%
    filter(u_grad > 10)
  view(sep_old)
...
sep_new <- 
  ...
  rename(id = course_id,
         ...
         department = course_department) %>% 
group_by(title, id, name, department) %>%
  summarize(new_total_ugrad = sum(u_grad), .groups = "drop_last") %>%
    filter(new_total_ugrad > 10)
  view(sep_new)
```
...
- Keep in mind missing data
- Keep in mind unobservable data


After we have a model, Temperance suggests we should:

- Keep in mind uncertainty in estimating the ATE
- Keep in mind individual variation
- Realize that randomization is impossible here
...
```{r}
sep_both <- inner_join(sep_old, sep_new, by = c("id", "name", "department", "title"))


sep_both %>%
  mutate(changes = new_total_ugrad - u_grad) %>%
